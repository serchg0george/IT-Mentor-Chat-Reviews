...
Почему? `static` означает, что поле принадлежит классу, а не объекту. Если ты создашь две симуляции, они будут использовать одни и те же данные. Используй `static` только для констант (например `private static final int MAX_SIZE = 100`).
